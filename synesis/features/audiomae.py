...
class AudioMAE(nn.Module):
    def __init__(self, feature_extractor=False, extract_kws={}, **kwargs):
        ...
        self.model = AutoModel.from_pretrained("hance-ai/audiomae", trust_remote_code=True)
    ...
    def encode(self, waveform, device):
        ...
        melspec = melspec[:,None,:,:]  # (1, 1, length, n_freq_bins) = (1, 1, 1024, 128)
        z = self.model.encoder.forward_features(melspec.to(device)).cpu()  # (b, 1+n, d); d=768
        z = z[:,1:,:]  # (b n d); remove [CLS], the class token

        b, c, w, h = melspec.shape  # w: temporal dim; h:freq dim
        wprime = round(w / self.model.encoder.patch_embed.patch_size[0])  # width in the latent space
        hprime = round(h / self.model.encoder.patch_embed.patch_size[1])  # height in the latent space
    ...
    def waveform_to_melspec(self, waveform:torch.FloatTensor):
        ...
        for i in range(b):
            ...
            if current_frames > expected_frames:
                ...
                mel_spectrogram_ = torch.nn.functional.pad(mel_spectrogram_, (0, 0,  # (left, right) for the 1st dim
            ...
            mel_spectrogram_ = (mel_spectrogram_ - self.model.encoder.MEAN) / (self.model.encoder.STD * 2)  # (length, n_freq_bins) = (1024, 128)
